...
export interface LiveVoiceDaemonContextOptions {
  ...
  logger: Logger;
  /** Base for `~`-rooted profile file paths. */
  paseoHome: string;
}
...
/**
 * Reads the profile's files into developer items. Each file is capped, the set
 * is capped, and whatever the daemon snapshot left of the provider's budget is
 * the ceiling; a file that cannot be read is logged and skipped rather than
 * failing the call.
 */
export async function loadUserContextItems(options: {
...
/** Only the fields the prompt builder reads, with absent ones dropped. */
function forwardedContextOptions(options: LiveVoiceContextBuildOptions | undefined) {
  return {
...
export class LiveVoiceDaemonContextProvider implements LiveVoiceContextProvider {
  ...
  async build(options?: LiveVoiceContextBuildOptions): Promise<LiveVoiceStartContext | null> {
    ...
    const userContextItems = options?.contextFiles?.length
      ? await loadUserContextItems({
          filePaths: options.contextFiles.map((file) =>
            resolveConfiguredPath(this.paseoHome, file),
          ),
          limits,
          snapshotItems: buildLiveVoiceInitialItems(snapshot, limits),
          logger: this.logger,
        })
      : [];
    const context = buildLiveVoiceStartContext(snapshot, {
      ...forwardedContextOptions(options),
      limits,
    ...
    this.logger.debug(
      {
        ...
        contextFileCount: userContextItems.length,
