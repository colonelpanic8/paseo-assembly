...
// Automatic host labels disambiguate which machine a workspace lives on once the
// visible sidebar spans more than one host. Counting distinct hosts across the visible
// projects (not all connected hosts) keeps automatic labels off when a host filter pins
// the view to a single host. The display preference bypasses that automatic policy.
export function shouldShowSidebarHostLabels(input: {
  projects: SidebarProjectEntry[];
